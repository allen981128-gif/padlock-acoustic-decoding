...
MIN_MOVABLE_BASE_THICKNESS_MM = 5.0
...
def move_occurrence_z(occ, dz):
    if not occ or abs(dz) < 1e-9:
        return
    ...
    try:
        mat = occ.transform2.copy()
    except Exception:
        mat = occ.transform.copy()

    tr = mat.translation
    mat.translation = adsk.core.Vector3D.create(tr.x, tr.y, tr.z + dz)
...
def run(context):
    ...
    try:
        ...
        if not slider_occ:
            raise RuntimeError("06 linear rail not found. Run 05/06 rail script first.")

        if not drive_occ:
            raise RuntimeError("07 drive head not found. Run 07 first.")

        if not servo_occ:
            raise RuntimeError("09 servo not found. Run 09 first.")
        ...
        min_base_t = mm(MIN_MOVABLE_BASE_THICKNESS_MM)
        current_gap = motor_bb.minPoint.z - base_z0

        # If the temporary reference stack leaves no printable platform thickness,
        # lift the complete lock/drive/servo contact group together. This preserves
        # roller-to-lock alignment while restoring a 5 mm moving platform.
        if current_gap < min_base_t:
            lift_dz = min_base_t - current_gap
            move_occurrence_z(drive_occ, lift_dz)
            move_occurrence_z(servo_occ, lift_dz)
            move_occurrence_z(find_occ(root, LOCK_OCC_PREFIX), lift_dz)

        # Refresh every world-space box after the optional lift.
        motor_bb = find_body_bbox(drive_occ, MOTOR_BODY_KEY)
        ...
        if base_z1 <= base_z0 + mm(1.0):
            raise RuntimeError(
                "Automatic lift did not create a usable moving-platform thickness."
            )
